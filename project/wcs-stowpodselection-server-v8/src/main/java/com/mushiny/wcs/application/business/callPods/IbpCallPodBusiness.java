 ...
 */
@Component
public class IbpCallPodBusiness extends  CommonCallPodBusiness {
    private static final Logger LOGGER = LoggerFactory.getLogger(IbpCallPodBusiness.class);
    @Autowired
    public IbpCallPodBusiness(PodRepository podRepository,
    ...
    public void setStationMaxPod(CallStation callStation){
        int stationMaxPod= systemPropertyBusiness.getProblemResolveStationMaxPod(callStation.getWorkStation().getWarehouseId());
        callStation.setWorkStationMaxPod(stationMaxPod);
    }
    @Override
    public void setStationMaxWorkLoad(CallStation callStation){
      int maxWorkLoad=systemPropertyBusiness.getProblemResolveStationMaxWorkLoad(callStation.getWorkStation().getWarehouseId());
        callStation.setWorkMaxWorkLoad(maxWorkLoad);
    }
    @Override
    public void setStationCycle(CallStation callStation){
       int cycle= systemPropertyBusiness.getProblemResolveStationCycle(callStation.getWorkStation().getWarehouseId());
    ...
    public boolean overMaxStationAvailablePod(CallStation callStation,CallPod callPod){
       ...
       int stationMaxPod=callStation.getWorkStationMaxPod();
    ...
    public boolean overMaxStationWork(CallStation callStation,CallPod callPod){
        boolean result=false;
        long systemMaxPsWorkLoad=callStation.getWorkMaxWorkLoad();
