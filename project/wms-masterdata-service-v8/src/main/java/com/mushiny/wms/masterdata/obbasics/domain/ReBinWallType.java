...
public class ReBinWallType extends BaseWarehouseAssignedEntity {
    ...
    @OrderBy("orderIndex")
    @OneToMany(mappedBy = "reBinWallType", cascade = {CascadeType.ALL}, orphanRemoval = true)
    private List<ReBinWallTypePosition> positions = new ArrayList<>();

    public void addPosition(ReBinWallTypePosition position) {
        getPositions().add(position);
        position.setReBinWallType(this);
    }
