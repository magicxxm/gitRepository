...
(function () {
    ...
    angular.module('myApp').controller("icqaDetailCtl", function ($scope, $window, $rootScope, commonService, ICQABaseService, icqaDetailService) {
        ...
        icqaDetailService.selectRoundOfInventory($scope.times, $scope.inventoryInformation, $scope.createdDateOne, $scope.createdDateTwo, $scope.type, function (data) {
        ...
        var columnsDetails = [
            {
                headerTemplate: "<span translate='选择'></span>",
                width: 35,
                template: "<input type=\"checkbox\"  ng-model='chk' id='dataItem.id' class='check-box' ng-checked = 'select_one' ng-click='selectOne(chk,dataItem.uid)'/>"
            },
        ...
        $scope.myGridChange = function () {
            ...
            // icqaDetailService.selectRoundOfInventoryId($scope.times, $scope.stocktakingId, $scope.createdDateOne, $scope.createdDateTwo, $scope.type, function (data) {
            //
            //     var grid = $("#stocktakingSelectGridONEId").data("kendoGrid");
            //     grid.setDataSource(new kendo.data.DataSource({data: data}));
            // });

            // var url = "icqa/stocktaking-orders/details-webs?stocktakingId=" + $scope.stocktakingId + "&times=" + $scope.times + "&type=" + $scope.type;
            // $scope.stocktakingDetailsGridOption = commonService.gridMushiny({
            //     columns: columnsDetails,
            //     dataSource: icqaDetailService.getGridDataSourceSpecial(url)
            // });
            // $scope.getdata($scope.stocktakingId, $scope.times, $scope.type);
            // icqaDetailService.select0rdersByStocktakingIds(data.id, $scope.times, $scope.type,function (res) {
        ...
        $scope.oneInventory = function () {
            ...
            if ($scope.stocktakingId) {
                ...
                // icqaDetailService.select0rdersByStocktakingIds($scope.stocktakingId, $scope.times, $scope.type, function (res) {
                //     var grid = $("#stocktakingDetailsGridId").data("kendoGrid");
                //     grid.setDataSource(new kendo.data.DataSource({data: res}));
                // });
                // debugger;
        ...
        $scope.twoInventory = function () {
            ...
            if ($scope.stocktakingId) {
                ...
                icqaDetailService.checkInventory($scope.stocktakingId, $scope.times, "daily", function (v) {
                    if (v.length > 0) {
        ...
        $scope.deletePersonnelArr = function (id) {
            ...
            if ($scope.stocktakingId) {
                ...
                icqaDetailService.deleteUsers($scope.stocktakingId, $scope.times, "daily", function () {
        ...
        $scope.closeStocktaking = function () {
            ...
            if (dataFiled.length > 0) {
                ...
                icqaDetailService.getCloseStocktaking(closeData, function (data) {
                    // var url = "icqa/stocktaking-orders/details-webs/page?stocktakingId=" + $scope.stocktakingId + "&times=" + $scope.times + "&type=" + $scope.type;
                    // $scope.stocktakingDetail = true;
                    // var grid = $("#stocktakingDetailsGridId").data("kendoGrid");
        ...
        $scope.getdata = function (stocktakingId, times, type) {
            ...
            var grid = $("#stocktakingDetailsGridId").data("kendoGrid");
                grid.setOptions(commonService.gridMushiny({
                    columns: columnsDetails,
                    dataSource: ICQABaseService.getGridDataSource1(url),
                    height: $(document.body).height() - 320
                })
            )
            // $scope.stocktakingDetailsGridOption = commonService.gridMushiny({
            //     columns: columnsDetails,
        ...
        $scope.getUserList = function (stocktakingId,times,type) {
            ...
            //     height: $(document.body).height() - 360
            // });
            icqaDetailService.getNotStocktakingAmount(stocktakingId,times,function (data) {
                debugger;
        ...
        function inventoryCount(id) {
            if (id) {
                icqaDetailService.selectInventoryCount(id, $scope.createdDateOne, $scope.createdDateTwo, $scope.type, function (data) {
