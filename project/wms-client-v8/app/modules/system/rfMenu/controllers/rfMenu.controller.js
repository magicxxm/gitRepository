...
(function(){
  ...
  angular.module('myApp').controller("rfMenuCtl", function ($scope, commonService, systemService, rfMenuService){
    // ==========================================================================================================
    ...
    // ==========================================================================================================
    // root
    rfMenuService.getRootModules(function(datas){
      $scope.listMenus = systemService.getJsonForListView({
        ...
        change: function(){
          ...
          rfMenuService.getRfMenu(curItem.id, function(datas){
            if(curItem.id != "ROOT" && !datas.length){
              getKendoList("listAssignModules").setDataSource(new kendo.data.DataSource({data: []}));
              getKendoList("listUnassignModules").setDataSource(new kendo.data.DataSource({data: []}));
            }else{
              // assignModules
              rfMenuService.getAssignedModules(curItem.id, function(assigned){
                getKendoList("listAssignModules").setDataSource(new kendo.data.DataSource({data: assigned}));
              });
              // unassignedModules
              rfMenuService.getUnassignedModules(curItem.id, function(unassigned){
    ...
    $scope.save = function(){
      ...
      rfMenuService.saveAssignedModules(curItem.id, items, function(){
