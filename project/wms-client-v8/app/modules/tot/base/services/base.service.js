  ...
  angular.module("myApp").service("totService", function($translate, commonService, TOT_CONSTANT){
    return {
      ...
      getDataSource: function (opts) {
        ...
        // grid数据源
        getGridDataSourceByJobDetail: function (params) {
            return {
                transport: {
                    read: function (options) {
                        // 取数据
                    ...
                    data: function (response) {
                        if (response.content.length >0) {
                            for (var i = 0; i < response.content.length; i++) {
                                response.content[i].entityLock = response.content[i].entityLock == 0 ? "正常" : (response.content[i].entityLock == 1 ? "问题" : "重包")
                            }
                        }
                        return response.content;
                    },
      ...
      // grid数据源
      getGridDataSource9: function (key){
        ...
        return {
          transport: {
            read: function(options){
              // 排序
              var sort = options.data.sort || [];
              var orderByField = options.data.sort || [];
              var isAsc = options.data.sort || new Boolean(true);
              sort && sort.length && (orderByField = sort[0].field)&&(sort[0].dir=='desc')&&(isAsc = new Boolean(false))
              // 过滤
              var filter = options.data.filter || {}, filterItems = filter.filters || [],
                  filterColumn = filterItems[0].field||[],filterValue = filterItems[0].value||[];
