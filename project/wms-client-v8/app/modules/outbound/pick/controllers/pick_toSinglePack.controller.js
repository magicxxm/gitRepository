(function () {
    ...
    angular.module('myApp').controller("pickToSinglePackCtl", function ($scope,$stateParams, $interval, $rootScope, $state, $window, FLOOR_COLOR,BACKEND_CONFIG, OUTBOUND_CONSTANT, pickToSinglePackService,pickToPackService, outboundService, webSocketService,$timeout) {
        ...
        $scope.pickPackOrderState = "订单状态";
        $scope.packWeightEnable = true;
        $scope.callNewEnable = false;
        $scope.problemEnable = false;
        $scope.damagedEnable = false;
        $scope.operation = "PICK SINGLE";
        ...
        $scope.scanStation = function (e) {
            ...
            pickToSinglePackService.scanStation($scope.stationNo, function (data) {
                if (data.msg == "none") {
                ...
                if (data.msg == "success") {
                    ...
                    $scope.websocketInter = $interval(function () {
                        if($scope.socketValue == "open"){
                            clearLight();
                            $scope.socketValue = "";
                            $interval.cancel($scope.websocketInter);
                            initMainPage1();
                        }
                    }, 100)
        ...
        $scope.continueUse = function () {
            ...
            if (size == 3) {
                ...
                pickToSinglePackService.checkPickedContainer($scope.stationNo, function (data) {
                    checkContainer(data);
                },function () {
                    bindStorageStep = 1;
                    selectBasket();
        ...
        function checkContainer(data) {
            ...
            $scope.podMessage = "POD";
                ...
                $scope.windowMessage = "请将商品放到电子称上，点击称重按钮进行称重";
                $scope.packOrderState = "weight";
                $scope.packWeightEnable = false;
                $scope.callNewEnable = true;
                $scope.problemEnable = true;
                $scope.damagedEnable = true;
                $scope.basketOperate = "";
                $scope.pickNo = data.pickFromStockUnit.amount;
                $scope.skuNo = "SKU: " + data.pickFromStockUnit.itemDataDTO.itemNo;//商品skuNo
                $scope.skuName = data.pickFromStockUnit.itemDataDTO.name;

                $scope.packBox = data.customerShipmentDTO.boxType ? data.customerShipmentDTO.boxType.name:"";
                $scope.pickPackOrderState = "订单："+data.customerShipmentDTO.shipmentNo;
                ...
                var box = data.customerShipmentDTO.boxType ? data.customerShipmentDTO.boxType.typeGroup : "";
                $scope.useBubbleFilm = data.pickFromStockUnit.itemDataDTO.useBubbleFilm
                if (box == "BOX") {
                    if ($scope.useBubbleFilm) {
                        $scope.useBubbleFilm = false;
                    }
                }
                var time = (new Date($scope.packExsdTime) - new Date) / 3600000;
                packExsdColorChange(time);
        }
        ...
        $scope.getOrderPosition = function (podNo, sectionId, stationNo) {
            //获取取货单信息
            pickToSinglePackService.getOrderPosition(podNo, sectionId, stationNo, function (data) {
        ...
        function getPickOrder(data) {
            ...
            pickToPackService.scanPod(data.socketPodName, function (data1) {
                ...
                $scope.item_good = data.itemDataDTO.itemNo;
                ...
                $scope.packOrderState = "picking";
                $scope.packBox = data.customerShipmentDTO.boxType ? data.customerShipmentDTO.boxType.name:"";
                $scope.pickPackOrderState = "订单："+data.customerShipmentDTO.shipmentNo;
                $scope.shipmentNo = data.customerShipmentDTO.shipmentNo;
                $scope.packExsdTime = kendo.format("{0:yyyy-MM-dd HH:mm:ss}", kendo.parseDate(data.customerShipmentDTO.deliveryDate));
                var box = data.customerShipmentDTO.boxType ? data.customerShipmentDTO.boxType.typeGroup : "";
                $scope.useBubbleFilm = data.itemDataDTO.useBubbleFilm
                if (box == "BOX") {
                    if ($scope.useBubbleFilm) {
                        $scope.useBubbleFilm = false;
                    }
          ...
          //根据距离发货时间改变exsd的颜色
        function packExsdColorChange(time) {
            if(time>12){
        ...
        function confirmItemData(amount) {
            ...
            pickToSinglePackService.confirmScanSKU(obj, function (data) {
        ...
        function checkProblemItemSuccess(type) {
            if ($scope.pickNo == 1) {
                if ($scope.serialNoCode == "ALWAYS_RECORD") {
                    ...
                    $scope.serialNoType = type;//扫描序列号变为残品序列号状态
        ...
        $scope.positionEmpry = function () {
            ...
            pickToPackService.haveScanedAllSKU($scope.pickId,$scope.operation, function (data) {
               // initOrderPosition();
                $scope.getOrderPosition($scope.podNo,$scope.sectionId,$scope.stationNo);
            });
        }
        //已扫描完所有商品
            ...
            pickToPackService.scanpickPackCellDigital($scope.cellLightName,function (data) {
                if($scope.pickPackCellLight == data.digitalId){
                    ...
                    if ($scope.shipmentState == 600) {
                        sendMessage($scope.pickPackCellLight2,$scope.shipmentId,$scope.pickPackWallId);
                    }
                    initOrderPosition();
                }else{
                    $scope.pickingOrderPostion = "digital";
        ...
        $scope.packGoodsWeight = function () {
            ...
            $scope.weight = 11;
            pickToSinglePackService.weigh($scope.shipmentNo, $scope.weight, function () {
                $scope.windowMessage = "请将下列商品装入指定的包装箱中，并扫描订单号码";
                $scope.packOrderState = "scanShipment";
                $scope.basketOperate = "checkShipment";
                $scope.packWeightEnable = true;
                $scope.step = "checkShipment";
                $scope.iptFocus();
            });
            //});
        };
        ...
        //扫描订单
        $scope.scanShipment = function (e) {
            var keycode = window.event ? e.keyCode : e.which;
            if (keycode == 13) {
                $scope.packErrorWindow.close();
                $scope.shipmentValue = $scope.shipment;
                if ($scope.shipmentNo == $scope.shipmentValue) {//扫描的shipment正确
                    $scope.windowMessage = "请扫描箱型号码";
                    $scope.step = "checkBox";
                    $scope.basketOperate = "checkBox";
                    $scope.packOrderState = "scanbox";
                    $scope.iptFocus()
                } else {
                    pickToSinglePackService.scanSingleShipment($scope.shipmentValue, function (){//扫描的shipment不正确，判断是否是商品
                        $scope.windowMessage = "请扫描箱型号码";
                        $scope.step = "checkBox";
                        $scope.basketOperate = "checkBox";
                        $scope.packOrderState = "scanbox";
                        ...
                        if(data.key == "EX_SHIPMENT_ISNOT_CURRENT_SHIPMENT"){
                            $scope.windowMessage = "请重新扫描订单号码";
                            $scope.errorMsg = $scope.shipmentValue + "并不是此订单对应的订单号码，请重新扫描"
                            $scope.packErrorStep = "checkShipment";
                            $scope.errorWindow("packErrorWindowId",$scope.packErrorWindow,function (){$("#checkShipmentTxtId").focus()});
                        }
        ...
        $scope.checkBox = function (e) {
            ...
            if (keycode == 13) {
                ...
                if($scope.boxName == null || $scope.boxName == undefined || $scope.boxName == ""){
                    $scope.packBoxWindow.close();
                    $scope.errorWindow("packBoxWindow", $scope.packBoxWindow);
                }else {
                    if ($scope.boxName.toLowerCase() == $scope.packBox.toLowerCase()) {
                        $scope.packFinish();
                    } else {
                        $scope.packBoxWindow.close();
                        $scope.errorWindow("packBoxWindow", $scope.packBoxWindow);
                ...
                /*if ($scope.boxName.toUpperCase() == $scope.packBox.toUpperCase()) {
                    $scope.packFinish();
                } else {
                    $scope.packBoxWindow.close();
                    $scope.errorWindow("packBoxWindow", $scope.packBoxWindow);
        ...
        $scope.packFinish = function () {
            $scope.packBoxWindow.close();
            $scope.packErrorWindow.close();
            pickToSinglePackService.packFinish($scope.shipmentNo, $scope.boxName,"SINGLE", function () {
                $scope.callNewEnable = false;
                $scope.problemEnable = false;
                $scope.damagedEnable = false;
                getlightOffPosition();
                ...
                $scope.errorWindow("packErrorWindowId",$scope.packErrorWindow,function (){$("#checkBoxTxtId").focus()});
            });
        };
        ...
        function initOrderPosition() {
          ...
          /*  $scope.itemNo = "";
            ...
            $scope.packBox = "";
            $scope.useBubbleFilm = false;
            $scope.pickPackOrderState = "";
            $scope.packOrderState = "";
            $scope.packExsdTime = "";
            $scope.packExsdColor = "#0066CD";
        ...
        //获取取货单信息
        $scope.getPickMessage = function (podNo, sectionId, stationNo) {
            //获取取货单信息
            pickToSinglePackService.getOrderPosition(podNo, sectionId, stationNo, function (data) {
                console.log(date.toLocaleString()+"->获取拣货单成功。。。");
                if(data.message == "nextPod"){
                    $scope.refreshNewPod();
                }else if(data.message == "noPod"){
                    $scope.podMessage = "POD";
                    $scope.podRows = [];
                    initMainCallPod();
                }else{
        ...
        $scope.refreshNewPod = function () {
            outboundService.refreshPod($scope.sectionId,$scope.workStationId,function (data) {
                console.log(date.toLocaleString()+"->刷新获取pod SUCCESS",data);
                callNewPod(data);
            },function (data) {
        ...
        //关闭websocket
      /*  function closePodWebsocket(){
            if(!($.isEmptyObject(podSocket))){
                console.log("pod客户端主动关闭websocket连接");
                podSocket.close(3666,"客户端主动关闭websocket连接");
            }
        };*/
        /*  function getPodResult() {
        ...
        $scope.iptFocus=function () {
            if($scope.step == 'checkGoods'){
                setTimeout(function () {
                    $("#checkGoodsTxt").focus();
                })
            }else if($scope.step == 'checkSerialNumber'){
                setTimeout(function () {
                    $("#checkSerialNumberTxt").focus();
                },0)
            }else if($scope.step == 'checkShipment'){
                setTimeout(function () {
                    $("#checkShipmentTxt").focus();
                },0)
            }else if($scope.step == 'checkBox'){
                setTimeout(function () {
                    $("#checkBoxTxt").focus();
                },0)
