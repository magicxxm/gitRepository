...
(function () {
  ...
  angular.module('myApp').controller("packScanShipmentCtl", function ($scope, $window, $state, $rootScope,$interval,FLOOR_COLOR,packService, outboundService, $translate) {
    ...
    $rootScope.packStepName = "请扫描Shipment ID，并取出商品放在电子称上";
    setTimeout(function () {
      $("#workstationId").focus();
    }, 0);
            ...
            $scope.getPickPackCellName = $interval(function () {
              if(lightStep == 1){
                  ...
                  packService.getCellName($scope.labelId, function (data) {
                      ...
                      //$scope.reBinCellName = "PPW_M_TEST-A04";
...
*/
      ...
      $scope.checkCell = function (e) {
          var keyCode=window.event ? e.keyCode : e.which;
          if(keyCode == 13) {
              $scope.getGoods($scope.pickPackCellName);
    ...
    $scope.getGoods = function (pickPackCellId) {
      ...
      packService.getGoodsbByShipment(pickPackCellId, $scope.stationName, "YES", function (data) {
        ...
        if ($scope.shipmentState != 800) {
          ...
          $scope.reBinCellName = data.storgeName;
          ...
          $scope.reBinCellColor = FLOOR_COLOR[$scope.rebinCell.substring(2, 3)];
    ...
    $scope.checkShipment = function (e) {
      ...
      if (keycode == 13) {
        ...
        if ($scope.shipmentNo == $scope.shipment) {//扫描的shipment正确
          ...
          packService.scanShimpment($scope.shipment,$scope.reBinCellName, function (data){//扫描的shipment不正确，判断是否是商品
              $scope.packProblem = "none";
             ...
             /* var grid = $("#packGoodDetailsGrid").data("kendoGrid");//获取表格中的商品的信息
              ...
              if(data.key == "EX_SHIPMENT_ISNOT_CURRENT_SHIPMENT"){
                ...
                $rootScope.errorMsg = data.values[0] + "并不是此订单对应的订单号码，请重新扫描"
