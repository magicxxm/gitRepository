 ...
 */
public class ZhongTongTemplateBuilder implements TemplateBuilder {
...
//70 38^B3N,N,50,Y,N,N
    ...
    /*            p.content=CONTROLHEAD+
       ...
       String linebarCode="^XA^FO50,20^GB400,0,4,^FS" +
        ...
        String deliveryNetwork="{deliveryNetwork}";

        String shipMentId = "{shipMentId}";

        String sender = "寄件人:{sender}";
        String senderPhone = "联系电话:{senderPhone}";
        String senderAddress = "地址:{senderAddress}";
        String sendDate="日期:{sendDate}";
        String receiver = "收件人:{receiver}";
        String receiverPhone = "联系电话:{receiverPhone}";
        String receiverAddress = "地址:{receiverAddress}";
        String commodityName = "品名:{commodityName}";
        String waybillNumber = "{waybillNumber}";
        Map<String, String> templateDetails = new HashMap<>();
        templateDetails.put("shipMentId", shipMentId);
        ...
        templateDetails.put("commodityName", commodityName);
        templateDetails.put("sendDate", sendDate);
        templateDetails.put("waybillNumber", waybillNumber);
        templateDetails.put("deliveryNetwork", deliveryNetwork);

        Template tm = new Template();
        tm.setTemplateDetail(templateDetails);
        tm.setTemplateType(TemplateType.ZHONG_TONG);
        return tm;
    }
}
